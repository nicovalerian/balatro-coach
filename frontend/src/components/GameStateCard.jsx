...
export default function GameStateCard({ state }) {
  ...
  const handSettings = sidebar.hand_settings ?? [];
  ...
  return (
    <div className="space-y-4">
      ...
      <Section title="Game State / Run Brief">
      ...
      <Section title="Hands Setting">
        <div className="space-y-2">
          {handSettings.map((hand) => (
            <div
              key={hand.name}
              className="flex items-center justify-between gap-3 rounded-[14px] border border-white/10 bg-black/18 px-3 py-2"
            >
              ...
              <div className="flex shrink-0 items-center gap-2">
                <span className="rounded-full border border-[#3498db]/55 bg-[#3498db]/16 px-2 py-1">
                  <span className="pixel-font text-[10px] text-[#d9efff]">{hand.chips}</span>
                </span>
                <span className="rounded-full border border-[#e43f3f]/55 bg-[#e43f3f]/16 px-2 py-1">
                  <span className="pixel-font text-[10px] text-[#ffd7d7]">x{hand.mult}</span>
                </span>
              </div>
            </div>
          ))}
